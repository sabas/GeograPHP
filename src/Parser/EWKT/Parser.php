...
namespace GeograPHP\Parser\EWKT;
...
class Parser
{
     ...
     {
         $this->tokenizer = new Tokenizer($ewkt);
         $this->srid = null;
         $this->with_z = false;
         $this->with_m = false;
         $this->is_3dm = false;
     ...
     private function parse_geometry($allow_srid)
     {
         ...
         if ($token == "SRID") {
             // Parse SRID=nnnn
             if (!$allow_srid) {
                 throw new FormatError('SRID not allowed here');
             }
             if ($this->tokenizer->get_next_token() != '=') {
                 throw new FormatError('Invalid SRID expression');
             }

             $this->srid = intval($this->tokenizer->get_next_token());
             if ($this->tokenizer->get_next_token() != ';') {
                 throw new FormatError('Invalid SRID separator');
             }
         ...
         if (isset($this->type_map[$type])) {
             ...
             throw new FormatError("Invalid geometry type: " . $type);
         }
     }
     ...
     private function parse_coords()
     {
         ...
         if ($x === null || $y === null) {
             throw new FormatError('Bad POINT format');
         }
         ...
         if ($this->is_3dm) {
             ...
             if ($m === null || $m == ',' || $m == ')') {
                 throw new FormatError('m component expected but not found');
             } else {
             ...
             if ($z === null) {
                 throw new FormatError('Incorrect termination of EWKT string');
             }
     ...
     {
         if ($this->tokenizer->get_next_token() != '(') {
             throw new FormatError('Invalid POINT');
         }
     ...
     {
         if ($this->tokenizer->get_next_token() != '(') {
             throw new FormatError('Invalid LINESTRING');
         }
     ...
     {
         if ($this->tokenizer->get_next_token() != '(') {
             throw new FormatError('Invalid Linear Ring');
         }
     ...
     {
         if ($this->tokenizer->get_next_token() != '(') {
             throw new FormatError('Invalid POLYGON');
         }
     ...
     */
     ...
     {
         if ($this->tokenizer->get_next_token() != '(') {
             throw new FormatError('Invalid MULTIPOINT');
         }
     ...
     {
         if ($this->tokenizer->get_next_token() != '(') {
             throw new FormatError('Invalid MULTILINESTRING');
         }
     ...
     {
         if ($this->tokenizer->get_next_token() != '(') {
             throw new FormatError('Invalid MULTIPOLYGON');
         }
     ...
     {
         if ($this->tokenizer->get_next_token() != '(') {
             throw new FormatError('Invalid GEOMETRYCOLLECTION');
         }
     ...
     private function parse_point_list($type)
     {
         ...
         return call_user_func(array('GeograPHP\\Geometry\\' . $type, 'from_points'), $points, $this->srid, $this->with_z, $this->with_m);
     }
}
